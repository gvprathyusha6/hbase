...
public class CloneSnapshotFromClientAfterSplittingRegionTestBase
  extends CloneSnapshotFromClientTestBase {
	...
	private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(CloneSnapshotFromClientTestBase.class);
  ...
  public void testCloneSnapshotAfterSplittingRegion() throws IOException, InterruptedException {
    ...
    try {
      ...
      CommonFSUtils.logFileSystemState(TEST_UTIL.getHBaseCluster().getMaster().getFileSystem(), TEST_UTIL.getHBaseCluster().getMaster().getDataRootDir(), LOG);      
    ...
    CommonFSUtils.logFileSystemState(TEST_UTIL.getHBaseCluster().getMaster().getFileSystem(), TEST_UTIL.getHBaseCluster().getMaster().getDataRootDir(), LOG);
      ...
      System.out.println("End - testCloneSnapshotAfterSplittingRegion before finall/del and is successful");
  ...
  public void testCloneSnapshotBeforeSplittingRegionAndDroppingTable()
    ...
    try {
      ...
      verifyRowCount(TEST_UTIL, clonedTableName, snapshot1Rows);
      System.out.println("this testCloneSnapshotBeforeSplittingRegionAndDroppingTable is successful!!");
      System.out.println("End before finally - testCloneSnapshotBeforeSplittingRegionAndDroppingTable");
    } finally {
      admin.catalogJanitorSwitch(true);
