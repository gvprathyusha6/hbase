...
//    mobStoreSFT.add(Collections.singleton(StoreFileInfo.createStoreFileInfoForHFile(conf, getFileSystem(), dstPath, true)));
  }
...
//	        ? HRegionFileSystem.openRegionFromFileSystem(conf, fs, MobUtils.getMobTableDir(conf, getTableName()), MobUtils.getMobRegionInfo(getTableName()), 
//	        		false)
//	        : HRegionFileSystem.createRegionOnFileSystem(conf, fs, MobUtils.getMobTableDir(conf, getTableName()), MobUtils.getMobRegionInfo(getTableName())));
//	StoreContext storeContext = StoreContext.getBuilder().withColumnFamilyDescriptor(getStoreContext().getFamily())
//    .withRegionFileSystem(regionFS)
//        .withFamilyStoreDirectoryPath(MobUtils.getMobFamilyPath(conf, getTableName(), getStoreContext().getFamily().getNameAsString()))
...
//	    // *** ADD DEBUG LOGGING ***
//	    
//	    LOG.info("MOB SFT Debug - Store context family dir: {}", storeContext.getFamilyStoreDirectoryPath());
