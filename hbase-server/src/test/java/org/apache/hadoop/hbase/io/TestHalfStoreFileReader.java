 ...
 * @throws InterruptedException 
  ...
  public void testHalfScanAndReseek() throws IOException, InterruptedException {
	...
	HRegionFileSystem splitAregionFS = HRegionFileSystem.create(conf, fs, new Path(root_dir), splitAHri);
	StoreContext splitAStoreContext = StoreContext.getBuilder().withColumnFamilyDescriptor(ColumnFamilyDescriptorBuilder.of("CF")).withFamilyStoreDirectoryPath(splitAPath).withRegionFileSystem(splitAregionFS).build();
    ...
    HRegionFileSystem splitBregionFS = HRegionFileSystem.create(conf, fs, new Path(root_dir), splitBHri);
	StoreContext splitBStoreContext = StoreContext.getBuilder().withColumnFamilyDescriptor(ColumnFamilyDescriptorBuilder.of("CF")).withFamilyStoreDirectoryPath(splitBPath).withRegionFileSystem(splitBregionFS).build();
