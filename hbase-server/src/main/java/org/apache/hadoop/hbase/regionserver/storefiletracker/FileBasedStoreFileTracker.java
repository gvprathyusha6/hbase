...
class FileBasedStoreFileTracker extends StoreFileTrackerBase {
  ...
  public HFileLink createHFileLink(TableName linkedTable, String linkedRegion, String hfileName,
    ...
    StoreFileInfo storeFileInfo = new StoreFileInfo(conf, fs, new Path(ctx.getFamilyStoreDirectoryPath(), HFileLink.createHFileLinkName(linkedTable, linkedRegion, hfileName)), hfileLink);
  ...
  @Override
	public Reference createAndCommitReference(Reference reference, Path path) throws IOException {
	  StoreFileInfo storeFileInfo = new StoreFileInfo(ctx.getRegionFileSystem().getFileSystem().getConf() , ctx.getRegionFileSystem().getFileSystem(), path, reference);
