    ...
    while (true) {
      ...
      if (flushDesc != null) {
        ...
        verifyData(secondaryRegion, 0, lastReplayed + 1, cq, families);
    	  ...
    	  System.out.println("secondary region name - " + secondaryRegion.getRegionInfo().getEncodedName());
        ...
        for (HStore store : secondaryRegion.getStores()) {
        	...
        	StoreFileTracker primarysft = StoreFileTrackerFactory.create(CONF, false, primatryStore.getStoreContext());
        	System.out.println("is it actually marked primary - " + primatryStore.isPrimaryReplicaStore());
        	System.out.println("files in primary sft load post compaction - " + primarysft.load().size());
