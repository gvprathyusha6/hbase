...
public class TestHFileLinkCleaner {
  ...
  private void createLink(StoreFileTracker sft, boolean createBackReference) throws IOException {
    ...
    hfileLink = sft.createHFileLink(hri.getTable(), hri.getEncodedName(), hfileName, createBackReference);
  ...
  public void testHFileLinkByRemovingReference() throws Exception {
    ...
    Path linkPath = new Path(familyLinkPath, HFileLink.createHFileLinkName(hri.getTable(), hri.getEncodedName(), hfileName));
    HStoreFile storeFile = new HStoreFile(new StoreFileInfo(conf, fs, linkPath, hfileLink), BloomType.NONE, null);
