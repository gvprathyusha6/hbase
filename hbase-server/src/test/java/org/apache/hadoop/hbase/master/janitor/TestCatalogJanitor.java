...
public class TestCatalogJanitor {
  ...
  public void testCleanMerge() throws IOException {
    ...
    Path parentaRef = createMergeReferenceFile(storedir, tabledir, td.getColumnFamilies()[0], merged, parenta);
    Path parentbRef = createMergeReferenceFile(storedir, tabledir, td.getColumnFamilies()[0], merged, parentb);
  ...
  private Path createMergeReferenceFile(Path storeDir, Path tableDir, ColumnFamilyDescriptor columnFamilyDescriptor, RegionInfo mergedRegion,
    ...
    HRegionFileSystem mergedRegionFS = HRegionFileSystem.create(fs.getConf(), fs, tableDir, mergedRegion);
	StoreContext storeContext = StoreContext.getBuilder().withColumnFamilyDescriptor(columnFamilyDescriptor).withFamilyStoreDirectoryPath(storeDir).withRegionFileSystem(mergedRegionFS).build();
  ...
  public void testCleanParent() throws IOException, InterruptedException {
    ...
    FileSystem fs = this.masterServices.getMasterFileSystem().getFileSystem();
    HRegionFileSystem regionFS = HRegionFileSystem.create(this.masterServices.getConfiguration(), fs, tabledir , splita);
	StoreContext storeContext = StoreContext.getBuilder().withColumnFamilyDescriptor(td.getColumnFamilies()[0]).withFamilyStoreDirectoryPath(storedir).withRegionFileSystem(regionFS).build();
	StoreFileTracker sft = StoreFileTrackerFactory.create(this.masterServices.getConfiguration(), false, storeContext);
    sft.createReference(ref, p);
  ...
  private Path createReferences(final MasterServices services, final TableDescriptor td,
    ...
    Path p = new Path(storedir, Long.toString(now) + "." + parent.getEncodedName());
    FileSystem fs = services.getMasterFileSystem().getFileSystem();
    HRegionFileSystem regionFS = HRegionFileSystem.create(services.getConfiguration(), fs, tabledir , daughter);
	StoreContext storeContext = StoreContext.getBuilder().withColumnFamilyDescriptor(td.getColumnFamilies()[0]).withFamilyStoreDirectoryPath(storedir).withRegionFileSystem(regionFS).build();
	StoreFileTracker sft = StoreFileTrackerFactory.create(services.getConfiguration(), false, storeContext);
    sft.createReference(ref, p);
    return p;
  }
